...
# 获取远程文件内容，不用落盘，以标准输出 + 管道方式直接运行
wget -O- http://xx.bashcommand.com/test.sh | sh 
# 重命名
wget -O filename.zip http://xxxxxx
...
wget -b http://xxxxxx
#测试下载链接
wget --spider http://xxxxxx
#重试次数
wget --tries=40 http://192.168.1.10/abc.zip    
#文件太大，存成多个小文件
wget -i http://192.168.1.10/abc.zip   
# 下载整站到本地的download文件夹
wget --mirror -p -k -P ./bilibili https://www.bilibili.com/
#下载一个网站的本地镜像
# -c 断点续传 , -r 递归下载 , --level 第一级, -np 不追溯至父目录
wget -c -r --level=1 -k -p -np  -P ./bilibili https://www.bilibili.com/

#下载信息存入日志
wget -o download.log http://192.168.1.10/abc.zip  
#下载指定格式文件
...
参考
---
- [wget](http://linux.51yip.com/search/wget)
